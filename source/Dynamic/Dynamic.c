...
typedef struct {
	...
	int numModules;
	int initialized;
	LLoadLibraryProc loadLibrary;
	LGetProcAddressProc getProcAddress;
	LFreeLibraryProc freeLibrary;
	void *userdata;
} MEMORYMODULE, *PMEMORYMODULE;

typedef BOOL(WINAPI *DllEntryProc)(HINSTANCE hinstDLL,
...
BuildImportTable(PMEMORYMODULE module)
{
	...
	if (directory->Size > 0) {
		...
		for (; !IsBadReadPtr(importDesc, sizeof(IMAGE_IMPORT_DESCRIPTOR)) && importDesc->Name; importDesc++) {
			...
			FARPROC *funcRef;
			LModule handle = module->loadLibrary((LPCSTR)(codeBase + importDesc->Name), module->userdata);
			if (handle == NULL) {
			...
			tmp = (LModule*)realloc(module->modules, (module->numModules + 1)*(sizeof(LModule)));
			if (tmp == NULL) {
			...
			for (; *thunkRef; thunkRef++, funcRef++) {
				if (IMAGE_SNAP_BY_ORDINAL(*thunkRef)) {
					*funcRef = (LFarProc)module->getProcAddress(handle, (LPCSTR)IMAGE_ORDINAL(*thunkRef), module->userdata);
				}
				else {
...
static LModule _LoadLibrary(LPCSTR filename, void *userdata)
{
	...
	return (LModule)result;
}

static FARPROC _GetProcAddress(LModule module, LPCSTR name, void *userdata)
{
	return (FARPROC)GetProcAddress((HMODULE)module, name);
}

static void _FreeLibrary(LModule module, void *userdata)
{
	FreeLibrary((HMODULE)module);
}

LMemoryModule LLoadLibrary(const void *data)
{
	return MemoryLoadLibraryEx(data, _LoadLibrary, _GetProcAddress, _FreeLibrary, NULL);
}

LMemoryModule LLoadLibraryEx(const void *data,
	LLoadLibraryProc loadLibrary,
	LGetProcAddressProc getProcAddress,
	LFreeLibraryProc freeLibrary,
	void *userdata)
...
FARPROC MemoryGetProcAddress(LMemoryModule module, LPCSTR name)
{
...
void LFreeLibrary(LMemoryModule mod)
{
	int i;
...
LMemoryResource MemoryFindResource(LMemoryModule module, LPCTSTR name, LPCTSTR type)
{
...
LMemoryResource LFindResourceEx(LMemoryModule module, LPCTSTR name, LPCTSTR type, WORD language)
{
	unsigned char *codeBase = ((PMEMORYMODULE)module)->codeBase;
...
DWORD MemorySizeofResource(LMemoryModule module, LMemoryResource resource)
{
...
LPVOID MemoryLoadResource(LMemoryModule module, LMemoryResource resource)
{
	unsigned char *codeBase = ((PMEMORYMODULE)module)->codeBase;
	PIMAGE_RESOURCE_DATA_ENTRY entry = (PIMAGE_RESOURCE_DATA_ENTRY)resource;
...
int
MemoryLoadString(LMemoryModule module, UINT id, LPTSTR buffer, int maxsize)
{
	return MemoryLoadStringEx(module, id, buffer, maxsize, DEFAULT_LANGUAGE);
}

int
MemoryLoadStringEx(LMemoryModule module, UINT id, LPTSTR buffer, int maxsize, WORD language)
{
	LMemoryResource resource;
