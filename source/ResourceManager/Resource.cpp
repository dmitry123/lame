...
Bool Resource::Load(StringC fileName) {
	...
	if (resourceType.IsAudio()) {
        ...
        if (!this->resourceLoader_.audioLoader_->Check(GetFileExtension(fileName))) {
            PostErrorMessage("Invalid file's extension (%s)", GetFileExtension(fileName));
        }
        ...
        if (!this->resourceLoader_.textureLoader_->Check(GetFileExtension(fileName))) {
            PostErrorMessage("Invalid file's extension (%s)", GetFileExtension(fileName));
        }
...
Bool Resource::Save(StringC fileName) {
	...
	if (resourceType != this->resourceLoader_.audioLoader_->GetInfo()->fileType) {
		...
		switch (resourceType) {
			case kResourceTypeJpg: resourceLoader_.textureLoader_ = new JpgLoader(*(JpgLoaderPtr)this->resourceLoader_.textureLoader_); break;
			case kResourceTypePng: resourceLoader_.textureLoader_ = new PngLoader(*(PngLoaderPtr)this->resourceLoader_.textureLoader_); break;
			case kResourceTypeTga: resourceLoader_.textureLoader_ = new TgaLoader(*(TgaLoaderPtr)this->resourceLoader_.textureLoader_); break;
			case kResourceTypeBmp: resourceLoader_.textureLoader_ = new BmpLoader(*(BmpLoaderPtr)this->resourceLoader_.textureLoader_); break;
			case kResourceTypeWav: resourceLoader_.audioLoader_ = new WavLoader(*(WavLoaderPtr)this->resourceLoader_.textureLoader_); break;
                default: break;
		}
